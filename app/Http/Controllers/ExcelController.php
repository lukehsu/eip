    ...
    public function diaryexcel()
    {
        ...
        */
        ...
        if ($check<1) 
        {
            ...
            {
                $message->to($to)->subject('今日業務無新增資料');
            });
        ...
        echo '讀入檔案:',pathinfo($inputFileName,PATHINFO_BASENAME);
        echo "<br>"; 
        echo '檔案格式:',$inputFileType,'<br/>'; 
        ...
        $allinforcount = $highestRow - 2 ;
        echo '總共匯入='.$allinforcount  ; 
        echo "<br>"; 
        ...
        { 
            ...
            {  
                ...
                print_r($info); 
                echo '<br />'; 
                if (substr($info['word14'],0,1)=='U' or substr($info['word14'],0,1)=='R' or $info['word14']=='B0157' ) 
                {
                    $usergroup['office'] = '保聯';
                }
                if ($usergroup['office']=='藥品') 
                {
                    $company='boragp';
                }
                elseif ($usergroup['office']=='醫院') 
                {
                    $company='borahp';
                }
                elseif ($usergroup['office']=='保健') 
                {
                    $usergroup['office'] = '通路';
                    $company='boraheal';
                }
                ...
                /*$brstore = DB::table('bigsangents')->where('customercode','=',$info['word4'])->where('income','=','Y')->count();
                if ($brstore>0) {
                    $company='borastore';
                    $usergroup['office'] = '保經';
                }*/
                $brstore = DB::table('bigsangents')->where('customercode','=',$info['word4'])->where('income','=','N')->count();
                if ($brstore>0) {
                    $company='';
                    $usergroup['office'] = '';
                }
            ...
            Mail::send('mail.mail', [], function($message) use ($to) 
            {
                $message->to($to)->subject('今日業務資料已新增');
            });
    ...
    public function healcome()
    {
        ...
        */
        ...
        if ($check<1) 
        {
            ...
            Mail::send('mail.mail', [], function($message) use ($to) 
            {
                $message->to($to)->subject('今日保健無新增資料');
            });
            echo  "<script type='text/javascript'>setTimeout(self.close(),10000);</script>"; 
        }
        else
        ...
        { 
            ...
            {  
                ...
                $usergroup = User::where('name','=',$info['word14'])->first();
                $info['word6'] = PHPExcel_Shared_Date::ExcelToPHP($info['word6']);
                $info['word6'] = date( "Y-m-d", $info['word6']) ;
                $alldatabase = new dailyreport ;
                $alldatabase->SalesType = $info['word0'];
                $alldatabase->OrderNo = $info['word1'];
                $alldatabase->LineNo = $info['word2'];
                ...
                elseif ($info['word15']=='保健員購') {
                     $usergroup['office']='保員';
                } 
                elseif ($info['word15']=='保健團購') {
                     $usergroup['office']='保團';
                } 
                else
                {    
                ...
                DB::table('reportheal')->insert(array('SalesType' => $info['word0']
                                                    , 'OrderNo' => $info['word1']
                                                    , 'Guino' => $info['word13']
                                                    , 'Cusno' => $info['word4']
                                                    , 'Cusname' => $info['word5']
            ...
            Mail::send('mail.mail', [], function($message) use ($to) 
            {
                $message->to($to)->subject('今日保健資料已新增');
            });
        //echo  "<script type='text/javascript'>setTimeout(self.close(),60000);</script>"; 
        }
    }
        ...
        */
        ...
        //注意highestColumnIndex的列數索引從0開始 
            ...
            {  
                ...
                $info['word1'] = str_replace('銷退：BB','',$info['word1']);
                if ($info['word11']=='林雪洪') {
                    $Salesgroup = '聯邦';
                }
                else
                ...
                /*$unstore = DB::table('bigsangents')->where('customercode','=',$info['word2'])->count();
                if ($unstore>0) {
                    $company='unistore';
                    $Salesgroup = '聯經';
                }*/
    ...
    public function boehringer()
    {
        ...
        //注意highestColumnIndex的列數索引從0開始 
            ...
            {  
                ...
                if ($Salesname=='金容'  and $info['word3']<>'A0195' or $Salesname=='平廷' and $info['word3']<>'A0195'  ) {
                    $Salesgroup ='裕經';
                    $Company='boehstore';
                }
                else
